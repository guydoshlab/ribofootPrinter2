...
def main(fasta_in,sam_in,outfile,smallsize,largesize, window, subset_list):
	...
	if subset_list != "none":
		...
		gene_list = set(selection["geneid"].tolist())
	...
	for read in samgen:
		...
		# For each read, store the corresponding gene's length info for normalization step below.
		if (read[1] == '0'):	    # mapped reads that are going to be counted	
			...
			# Each read also counted according to overall transcript length in addition to being counted above in the "if" statement.
	...
	# If there are no reads mapping to a particular feature, then the normalization is set as undefined.
	...
	if len(startcodon_list) == 0:
		print(str(mappedreads["UTR5"])+" reads mapped within the 5'-UTR region.")
	else:
	...
	if len(CDS_list) == 0:
		print(str(mappedreads["UTR5"])+" reads mapped within the CDS region.")
	else:
	...
	if len(UTR3_list) == 0:
		print(str(mappedreads["UTR3"])+" reads mapped within the 3'-UTR region.")
	else:
	...
	if len(length_list) == 0:
		print(str(mappedreads["total"])+" reads mapped to the transcriptome (includes any subsetting).")
	else:
		total_norm = sum(length_list) / len(length_list)
		print(str(mappedreads["total"])+" reads mapped to the transcriptome (includes any subsetting) which has a weighted average transcript length of " + str(round(total_norm)) + " nt.")

# Normalize
	if mappedreads["UTR5"] == 0:	
		print("Warning: No reads in 5'-UTR region. Normalization cannot be performed.")
		normtable["probability_density"][0]=""
		normtable["rpm_density"][0]=""
		normtable["rpm_length_density"][0]=""	
	else:
	...
	if mappedreads["start"] == 0:	
		...
		normtable["probability_density"][1]=""
		normtable["rpm_density"][1]=""
		normtable["rpm_length_density"][1]=""
	else:
	...
	if mappedreads["CDS"] == 0:	
		...
		normtable["probability_density"][2]=""
		normtable["rpm_density"][2]=""
		normtable["rpm_length_density"][2]=""
	else:
	...
	if mappedreads["stop"] == 0:	
		...
		normtable["probability_density"][3]=""
		normtable["rpm_density"][3]=""
		normtable["rpm_length_density"][3]=""
	else:
	...
	if mappedreads["UTR3"] == 0:	
		...
		normtable["probability_density"][4]=""
		normtable["rpm_density"][4]=""
		normtable["rpm_length_density"][4]=""
	else:
	...
	if mappedreads["total"] == 0:	
		...
		normtable["probability_density"][5]=""
		normtable["rpm_density"][5]=""
		normtable["rpm_length_density"][5]=""
	else:
